...
golem_status co_markdown(struct json_object *r,golem_execution_reply *out)
{
    ...
    fprintf(f,"# Completion %llu\n\nWork: `%s`\n\nRecorded at Unix seconds: %llu\n\n",
        (unsigned long long)dw_uint(r,"sequence"),dw_text(a,"work_id"),(unsigned long long)dw_uint(r,"completed_at_unix_seconds"));
    fputs("## Goal\n\n",f); prose(f,dw_text(a,"goal"));
    fprintf(f,"\n\n## Engine Assessment\n\nDeclared development gates passed for this recorded snapshot.\n\nPolicy: `%s`\n\nEvidence root: `%s`\n\n",dw_text(a,"policy_digest"),dw_text(r,"evidence_root"));
    ...
    for(size_t i=0;i<json_object_array_length(decisions);++i) {
        ...
        fprintf(f,"- %s: %s. ",wf_stages[i],dw_text(d,"status")); prose(f,dw_text(d,"reason")); fputc('\n',f);
    }
    ...
    for(size_t i=0;i<json_object_array_length(definitions);++i) {
        struct json_object *def=json_object_array_get_idx(definitions,i),*cases=dw_get(def,"cases");
        for(size_t j=0;j<json_object_array_length(cases);++j) {
            struct json_object *c=json_object_array_get_idx(cases,j);
            fprintf(f,"| %s | %s | %s | PASS |\n",dw_text(c,"requirement_id"),dw_text(def,"id"),dw_text(c,"id"));
        }
    }
    ...
    if(!json_object_array_length(issues)) fputs("None declared; absence of undiscovered issues is not established.\n",f);
    for(size_t i=0;i<json_object_array_length(issues);++i) {
        struct json_object *v=json_object_array_get_idx(issues,i);
        fprintf(f,"- %s: ",dw_text(v,"id")); prose(f,dw_text(v,"description"));
        fprintf(f," (evidence `%s`)\n",dw_text(v,"evidence_digest"));
    }
    fprintf(f,"\n## Recovery Boundary\n\nReentry decisions: %llu. Session sequence: %llu.\n\nNo unresolved recorded dispatch or active claim at issuance.\nNo external-effect exactly-once guarantee. Recovery never reruns commands.\n",
        (unsigned long long)dw_uint(a,"reentry_count"),(unsigned long long)dw_uint(dw_get(r,"boundary"),"session_sequence"));
...
golem_status golem_completion_report(golem_document_store *s,uint32_t seq,bool project,golem_execution_reply *out,golem_diagnostic *d)
{
    if(!s || !out || !seq || seq>s->completion_count || s->poisoned) return dw_report(d,GOLEM_ERR_INVALID_ARGUMENT,NULL);
    if(project && !s->writable) return dw_report(d,GOLEM_ERR_POLICY_DENIED,NULL);
    struct json_object *event=s->completions[seq-1]; golem_digest key;
    if(!dw_digest(event,"report_digest",&key)) return dw_report(d,GOLEM_ERR_PARSE,NULL);
    uint8_t *bytes=NULL; size_t n=0;
    golem_status st=golem_evidence_read(s->cas,&key,GOLEM_DOCUMENT_MAX_BODY,NULL,&bytes,&n,NULL);
    if(project) {
        struct json_object *docs=dw_get(dw_get(dw_get(event,"record"),"assessment"),"documents");
        ...
        int top=-1,dir=-1; char name[32]; (void)snprintf(name,sizeof(name),"r%04u",seq);
        if(st==GOLEM_OK) st=dw_dir(s->root,"completions",true,&top);
        if(st==GOLEM_OK) st=dw_dir(top,name,true,&dir);
        if(st==GOLEM_OK) st=dw_publish(dir,"completion.md",(golem_bytes){bytes,n});
        if(dir>=0) close(dir);
        if(top>=0) close(top);
    }
