...
static golem_status root_hash(struct json_object *assessment, struct json_object *boundary,
                              golem_digest *out)
{
    struct json_object *o = json_object_new_object();
    golem_status st = GOLEM_OK;
    if (!dw_add(o, "assessment", json_object_get(assessment)) ||
        !dw_add(o, "boundary", json_object_get(boundary)))
                      ...
                      const golem_digest *payload, const golem_digest *frame)
{
    ...
    const char *rk[] = {"schema_version", "sequence", "event_sequence", "completed_at_unix_seconds",
                        "assessment",     "boundary", "evidence_root"};
    struct json_object *r = dw_get(event, "record"), *expected = NULL;
    golem_digest report, root, declared;
    if (!dw_keys(event, keys, 5) || dw_uint(event, "schema_version") != 1 ||
        strcmp(dw_text(event, "type"), "completion") || !dw_keys(r, rk, 7) ||
        dw_uint(r, "schema_version") != 1 || !dw_uint(r, "completed_at_unix_seconds") ||
        dw_uint(r, "completed_at_unix_seconds") > INT64_MAX ||
        dw_uint(r, "sequence") != s->completion_count + 1 ||
        dw_uint(r, "event_sequence") != s->event_count + 1 ||
        s->completion_count >= CO_MAX_RECORDS || !dw_digest(event, "report_digest", &report) ||
        !dw_digest(r, "evidence_root", &declared))
        return GOLEM_ERR_CORRUPT_JOURNAL;
    for (size_t i = 0; i < s->completion_count; ++i)
        if (!strcmp(dw_text(dw_get(s->completions[i], "request"), "key"),
                    dw_text(dw_get(event, "request"), "key")))
            return GOLEM_ERR_CORRUPT_JOURNAL;
    struct json_object *boundary = dw_get(r, "boundary"), *attempts = dw_get(boundary, "attempts");
    const char *bk[] = {"session_sequence", "session_head", "session", "attempts"};
    golem_digest session_head;
    if (!dw_keys(boundary, bk, 4) ||
        dw_uint(boundary, "session_sequence") > GOLEM_AGENT_MAX_EVENTS ||
        !dw_digest(boundary, "session_head", &session_head) ||
        dw_get(dw_get(boundary, "session"), "active") || !ds_array(attempts, 0, 256))
        return GOLEM_ERR_CORRUPT_JOURNAL;
    for (size_t i = 0; i < json_object_array_length(attempts); ++i) {
        struct json_object *a = json_object_array_get_idx(attempts, i);
        const char *ak[] = {"attempt_id", "state"};
        if (!dw_keys(a, ak, 2) || !dw_id(dw_text(a, "attempt_id")) ||
            strcmp(dw_text(a, "state"), "SUBMITTED"))
            return GOLEM_ERR_CORRUPT_JOURNAL;
    }
    golem_status st = co_evaluate_record(s, dw_get(event, "request"), r, &expected);
    if (st == GOLEM_OK && !json_object_equal(expected, dw_get(r, "assessment")))
        st = GOLEM_ERR_CORRUPT_JOURNAL;
    if (st == GOLEM_OK)
        st = root_hash(expected, dw_get(r, "boundary"), &root);
    if (st == GOLEM_OK && !dw_equal(&root, &declared))
        st = GOLEM_ERR_DIGEST_MISMATCH;
    if (st == GOLEM_OK)
        st = co_boundary_verify(s, boundary);
    ...
    if (st == GOLEM_OK)
        st = co_markdown(r, &md);
    if (st == GOLEM_OK)
        st = golem_digest_bytes((golem_bytes){md.data, md.size}, &mdkey);
    if (st == GOLEM_OK && !dw_equal(&mdkey, &report))
        st = GOLEM_ERR_DIGEST_MISMATCH;
    if (st == GOLEM_OK)
        st = golem_evidence_verify(s->cas, &report, &size, NULL);
    if (st == GOLEM_OK) {
        s->completions[s->completion_count] = json_object_get(event);
        s->completion_digests[s->completion_count++] = *payload;
        s->last = *frame;
        ++s->event_count;
    }
...
{
    struct json_object *event = s->completions[index],
                       *docs = dw_get(dw_get(dw_get(event, "record"), "assessment"), "documents");
    golem_status st = GOLEM_OK;
    ...
    int top = -1, dir = -1;
    char name[32];
    (void)snprintf(name, sizeof(name), "r%04u", (unsigned)(index + 1));
    if (st == GOLEM_OK)
        st = dw_dir(s->root, "completions", false, &top);
    if (st == GOLEM_OK)
        st = dw_dir(top, name, false, &dir);
    uint8_t *bytes = NULL;
    size_t n = 0;
    golem_digest actual, expected;
    if (st == GOLEM_OK)
        st = dw_read_at(dir, "completion.md", GOLEM_DOCUMENT_MAX_BODY, &bytes, &n);
    if (st == GOLEM_OK)
        st = golem_digest_bytes((golem_bytes){bytes, n}, &actual);
    if (st == GOLEM_OK &&
        (!dw_digest(event, "report_digest", &expected) || !dw_equal(&actual, &expected)))
        st = GOLEM_ERR_DIGEST_MISMATCH;
    free(bytes);
    if (dir >= 0)
        close(dir);
...
static size_t latest(golem_document_store *s, const char *id)
{
    for (size_t i = s->completion_count; i > 0; --i)
        if (!strcmp(dw_text(dw_get(s->completions[i - 1], "request"), "selection_id"), id))
            return i - 1;
    return SIZE_MAX;
}
golem_status co_hint(golem_document_store *s, const char *id, const char **action)
{
    *action = NULL;
    ...
    if (st == GOLEM_ERR_STALE_RESULT || st == GOLEM_ERR_REQUIREMENTS_UNMET ||
        st == GOLEM_ERR_NOT_FOUND) {
        *action = "REVALIDATE_COMPLETION";
        st = GOLEM_OK;
    } else if (st == GOLEM_OK) {
        if (!json_object_equal(observed, dw_get(dw_get(event, "record"), "assessment")))
            *action = "REVALIDATE_COMPLETION";
        else {
    ...
    json_object_put(boundary);
    return st;
}
static golem_status receipt_value(struct json_object *event, const golem_digest *digest,
                                  golem_execution_reply *out)
{
    struct json_object *o = json_object_new_object();
    golem_status st = GOLEM_OK;
    if (!dw_add_digest(o, "receipt_digest", digest) ||
        !dw_add(o, "record", json_object_get(event)) || !ex_text(o, "state", "RECORDED") ||
        !ex_text(o, "next_action", "RESUME_TO_VERIFY_PROJECTION"))
        st = GOLEM_ERR_OUT_OF_MEMORY;
    if (st == GOLEM_OK)
        st = ex_emit(o, out);
    json_object_put(o);
...
static golem_status resume(golem_document_store *s, const char *id, golem_execution_reply *out)
{
    ...
    struct json_object *boundary = NULL, *o = json_object_new_object(), *next = NULL;
    golem_status st = co_quiescent(s, &action, &boundary);
    if (st == GOLEM_OK && !action)
        st = co_hint(s, id, &action);
    if (st == GOLEM_OK && !action) {
        size_t n = 0;
        st = golem_workflow_next(s, id, NULL, 0, &n, NULL);
        if (st == GOLEM_ERR_BUFFER_TOO_SMALL) {
            uint8_t *bytes = NULL;
            st = dw_scratch(s, n, &bytes);
            if (st == GOLEM_OK)
                st = golem_workflow_next(s, id, bytes, n, &n, NULL);
            if (st == GOLEM_OK)
                st = golem_json_parse((golem_bytes){bytes, n}, GOLEM_DOCUMENT_MAX_JSON, &next);
            dw_scratch_free(s, bytes);
        }
        if (st == GOLEM_OK)
            action = dw_text(next, "action");
    }
    struct json_object *docs = json_object_new_array(), *history = json_object_new_array(),
                       *results = json_object_new_array();
    if (!o || !docs || !history || !results)
        st = GOLEM_ERR_OUT_OF_MEMORY;
    wf_graph g = {0};
    if (st == GOLEM_OK)
    ...
    for (size_t i = 0; st == GOLEM_OK && i < s->count; ++i)
        if (g.states[i] == GOLEM_DOCUMENT_CURRENT) {
            ...
            if (!ex_text(v, "kind", dw_text(s->entries[i].meta, "kind"))) {
                json_object_put(v);
                st = GOLEM_ERR_OUT_OF_MEMORY;
            } else if (!wf_append(docs, v))
                st = GOLEM_ERR_OUT_OF_MEMORY;
            if (st == GOLEM_OK && dw_uint(s->entries[i].meta, "schema_version") == 5) {
                golem_digest key;
                struct json_object *r = NULL, *item = json_object_new_object();
                if (!dw_digest(s->entries[i].meta, "execution_receipt", &key))
                    st = GOLEM_ERR_PARSE;
                if (st == GOLEM_OK)
                    st = ex_load(s, &key, NULL, &r);
                golem_status fresh = st == GOLEM_OK ? ex_live(s, s->entries[i].meta) : st;
                if (fresh != GOLEM_OK && fresh != GOLEM_ERR_STALE_RESULT)
                    st = fresh;
                if (st == GOLEM_OK &&
                    (!dw_add_digest(item, "receipt_digest", &key) ||
                     !ex_text(item, "kind", dw_text(s->entries[i].meta, "kind")) ||
                     !ex_text(item, "status", dw_text(r, "status")) ||
                     !ex_text(item, "freshness", fresh == GOLEM_OK ? "CURRENT" : "STALE")))
                    st = GOLEM_ERR_OUT_OF_MEMORY;
                if (st == GOLEM_OK) {
                    if (!wf_append(results, item))
                        st = GOLEM_ERR_OUT_OF_MEMORY;
                } else
    ...
    size_t i = latest(s, id);
    if (st == GOLEM_OK &&
        (!ex_uint(o, "schema_version", 1) || !ex_text(o, "action", action) ||
         !ex_uint(o, "generation", s->count + 1) ||
         json_object_object_add(o, "next", json_object_get(next)) != 0 ||
         !dw_add(o, "boundary", json_object_get(boundary)) ||
         !dw_add(o, "current_documents", json_object_get(docs)) ||
         !dw_add(o, "failure_history", json_object_get(history)) ||
         !dw_add(o, "result_evidence", json_object_get(results)) ||
         !dw_add(o, "selection", wf_ref(plan)) ||
         ...
         !ex_uint(o, "completion_sequence", i == SIZE_MAX ? 0 : i + 1)))
        st = GOLEM_ERR_OUT_OF_MEMORY;
    if (st == GOLEM_OK && i != SIZE_MAX &&
        (!dw_add_digest(o, "receipt_digest", &s->completion_digests[i]) ||
         !dw_add(o, "completion", json_object_get(s->completions[i]))))
        st = GOLEM_ERR_OUT_OF_MEMORY;
    if (st == GOLEM_OK)
        st = ex_emit(o, out);
    wf_graph_free(s, &g);
...
static golem_status completion_permission(golem_document_store *store)
{
    ...
    if (store->completion_count >= CO_MAX_RECORDS)
        return GOLEM_ERR_BUDGET_EXHAUSTED;
    return re_deadline(store);
}

static golem_status completion_record(golem_document_store *store, struct json_object *assessment,
                                      struct json_object *boundary, const golem_digest *root,
                                      struct json_object **out)
{
    ...
    if (!ex_uint(record, "schema_version", 1) ||
        !ex_uint(record, "sequence", store->completion_count + 1) ||
        !ex_uint(record, "event_sequence", store->event_count + 1) ||
        !ex_uint(record, "completed_at_unix_seconds", (uint64_t)now) ||
        !dw_add(record, "assessment", json_object_get(assessment)) ||
        !dw_add(record, "boundary", json_object_get(boundary)) ||
        !dw_add_digest(record, "evidence_root", root)) {
                                   ...
                                   golem_execution_reply *out, golem_diagnostic *d)
{
    ...
    struct json_object *r = NULL, *assessment = NULL, *boundary = NULL, *record = NULL,
                       *event = NULL;
    ...
    if (st == GOLEM_OK && read) {
        st = resume(s, dw_text(r, "selection_id"), out);
        json_object_put(r);
    ...
    if (st == GOLEM_OK)
        st = co_evaluate(s, r, true, &assessment);
    for (size_t i = 0; st == GOLEM_OK && i < s->completion_count; ++i) {
        struct json_object *old = dw_get(s->completions[i], "request");
    ...
    golem_digest root, payload, frame;
    golem_execution_reply md = {0}, encoded = {0}, response = {0};
    golem_receipt report;
    if (st == GOLEM_OK)
        st = root_hash(assessment, boundary, &root);
    if (st == GOLEM_OK)
        st = completion_record(s, assessment, boundary, &root, &record);
    if (st == GOLEM_OK)
        st = co_markdown(record, &md);
    if (st == GOLEM_OK)
        st = golem_evidence_put(s->cas, (golem_bytes){md.data, md.size}, &report, NULL);
    if (st == GOLEM_OK) {
        ...
        if (!ex_uint(event, "schema_version", 1) || !ex_text(event, "type", "completion") ||
            ...
            !dw_add(event, "record", json_object_get(record)) ||
            !dw_add_digest(event, "report_digest", &report.digest))
            st = GOLEM_ERR_OUT_OF_MEMORY;
    }
    if (st == GOLEM_OK)
