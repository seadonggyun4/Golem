...
golem_status co_evaluate_v1(golem_document_store *s, struct json_object *r, bool live,
                            struct json_object **out)
{
    golem_status st = co_validate_v1(r);
    ...
    for (size_t i = 0; st == GOLEM_OK && i < json_object_array_length(docs); ++i) {
        ...
        if (isqa || isdev) {
            ...
            if ((isqa && qa) || (isdev && dev)) {
                st = GOLEM_ERR_INVALID_STATE;
                break;
            }
            golem_digest *key = isqa ? &qa_key : &dev_key;
            if (!dw_digest(e->meta, "execution_receipt", key))
                st = GOLEM_ERR_PARSE;
            if (st == GOLEM_OK)
                st = ex_load(s, key, isqa ? "qa" : "development", isqa ? &qa : &dev);
            if (st == GOLEM_OK && live)
                st = ex_live(s, e->meta);
        }
    }
    if (st == GOLEM_OK && (!qa || !dev || strcmp(dw_text(qa, "status"), "PASS") ||
                           ...
                           !json_object_equal(dw_get(qa, "checkpoint"), dw_get(dev, "checkpoint"))))
        st = GOLEM_ERR_REQUIREMENTS_UNMET;
    if (st == GOLEM_OK && !dw_digest(qa, "checkpoint", &cp_key))
        st = GOLEM_ERR_PARSE;
    if (st == GOLEM_OK)
        st = ex_load(s, &cp_key, "checkpoint", &cp);
    struct json_object *gates = dw_get(qa, "gates"),
                       *definitions = dw_get(dw_get(cp, "contract"), "gates");
    if (st == GOLEM_OK &&
        (!json_object_array_length(gates) ||
         json_object_array_length(gates) != json_object_array_length(definitions)))
        st = GOLEM_ERR_REQUIREMENTS_UNMET;
    ...
    for (size_t i = 0; st == GOLEM_OK && i < json_object_array_length(issues); ++i) {
        ...
        golem_digest key;
        uint64_t size;
        if (json_object_get_boolean(dw_get(v, "blocking")))
            st = GOLEM_ERR_REQUIREMENTS_UNMET;
        if (st == GOLEM_OK && !dw_digest(v, "evidence_digest", &key))
            st = GOLEM_ERR_PARSE;
        if (st == GOLEM_OK)
    ...
    if (st == GOLEM_OK)
        st = rs_outcome_completion(s, dw_text(r, "selection_id"), &qa_key, &outcomes);
    struct json_object *policy = json_object_new_object();
    golem_digest pd;
    if (!policy || !ex_uint(policy, "schema_version", 1) ||
        !ex_text(policy, "predicate", CO_EVALUATOR_V1) ||
    ...
    if (st == GOLEM_OK) {
        ...
        if (!ex_text(assessment, "work_id", dw_text(s->spec, "work_id")) ||
            ...
            !ex_uint(assessment, "generation", s->count + 1) ||
            !dw_add(assessment, "selection", wf_ref(plan)) ||
            !dw_add(assessment, "scope",
                    json_object_get(dw_get(dw_get(plan->meta, "selection"), "scope"))) ||
            !dw_add(assessment, "documents", json_object_get(docs)) ||
            !dw_add(assessment, "stage_decisions",
                    json_object_get(dw_get(dw_get(plan->meta, "selection"), "decisions"))) ||
            !dw_add(assessment, "snapshot", json_object_get(dw_get(qa, "snapshot"))) ||
            !dw_add(assessment, "gates", json_object_get(gates)) ||
            !dw_add_digest(assessment, "qa_receipt", &qa_key) ||
            !dw_add(assessment, "gate_definitions", json_object_get(definitions)) ||
            !dw_add(assessment, "requirement_ids",
                    json_object_get(dw_get(plan->meta, "requirement_ids"))) ||
            !dw_add_digest(assessment, "development_receipt", &dev_key) ||
            !dw_add_digest(assessment, "checkpoint", &cp_key) ||
            !dw_add(assessment, "policy", json_object_get(policy)) ||
            !dw_add_digest(assessment, "policy_digest", &pd) ||
            ...
            !ex_text(assessment, "assurance", "DECLARED_GATES_ONLY"))
            st = GOLEM_ERR_OUT_OF_MEMORY;
    }
    if (st == GOLEM_OK && json_object_array_length(outcomes) &&
        !dw_add(assessment, "outcome_adjudications", json_object_get(outcomes)))
        st = GOLEM_ERR_OUT_OF_MEMORY;
