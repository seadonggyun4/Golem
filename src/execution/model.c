...
golem_status ex_hash(struct json_object *o, golem_digest *out)
{
    ...
    return p ? golem_digest_bytes((golem_bytes){(const uint8_t *)p, strlen(p)}, out)
...
golem_status ex_emit(struct json_object *o, golem_execution_reply *out)
{
    ...
    if (!copy)
        return GOLEM_ERR_OUT_OF_MEMORY;
    memcpy(copy, p, n);
    *out = (golem_execution_reply){copy, n};
    return GOLEM_OK;
}
...
static bool string_array(struct json_object *a, size_t min, size_t max, bool paths)
{
    ...
    for (size_t i = 0; i < json_object_array_length(a); ++i) {
        ...
        if (!*p || strlen(p) > 2048 || (paths && !ds_path(p)))
            return false;
        for (const char *q = p; *q; ++q)
            if ((unsigned char)*q < 32)
                return false;
    }
    return true;
}
golem_status ex_contract(struct json_object *o)
{
    ...
    for (size_t i = 0; i < json_object_array_length(gates); ++i) {
        ...
        for (size_t j = 0; j < json_object_array_length(repos); ++j)
            if (strcmp(dw_text(json_object_array_get_idx(repos, j), "id"),
                       dw_text(g, "repository")) == 0)
        ...
        for (size_t j = 0; j < json_object_array_length(paths); ++j) {
            ...
            for (size_t k = 0; k < json_object_array_length(all); ++k)
                if (json_object_equal(json_object_array_get_idx(paths, j),
                                      json_object_array_get_idx(all, k)))
                    found = true;
            if (!found)
                return GOLEM_ERR_REQUIREMENTS_UNMET;
        }
        for (size_t j = 0; j < json_object_array_length(cases); ++j) {
            ...
            if (!dw_keys(c, ck, 2) || !dw_id(dw_text(c, "id")) ||
                !dw_id(dw_text(c, "requirement_id")))
                return GOLEM_ERR_PARSE;
            for (size_t k = 0; k < j; ++k)
                if (strcmp(dw_text(c, "id"), dw_text(json_object_array_get_idx(cases, k), "id")) ==
                    0)
                    return GOLEM_ERR_PARSE;
        }
        for (size_t j = 0; j < i; ++j)
            if (strcmp(dw_text(g, "id"), dw_text(json_object_array_get_idx(gates, j), "id")) == 0)
                return GOLEM_ERR_PARSE;
    }
    return GOLEM_OK;
}
golem_status golem_execution_contract_validate(golem_bytes b, golem_digest *digest,
                                               golem_diagnostic *d)
{
    if (!digest)
...
golem_status ex_current(golem_document_store *s, struct json_object *manifest)
{
    ...
    for (size_t i = 0; st == GOLEM_OK && i < json_object_array_length(docs); ++i) {
        ...
        if (!e || !dw_digest(r, "digest", &key) || !dw_equal(&key, &e->result.manifest_digest) ||
            ...
            uint64_t bytes;
            st = wf_integrity(s, (size_t)(e - s->entries), &bytes);
        }
    }
    wf_graph_free(s, &g);
    return st;
                       ...
                       struct json_object **out)
{
    ...
    if (loaded == GOLEM_OK && active && strcmp(dw_text(active, "kind"), kind) == 0) {
        golem_digest key;
        if (!dw_digest(active, "manifest_digest", &key))
            loaded = GOLEM_ERR_PARSE;
        else
            loaded = dw_cas_json(s, &key, &pinned);
        if (loaded == GOLEM_OK)
            budget = dw_uint(pinned, "byte_budget");
    }
    ...
    if (loaded != GOLEM_OK)
        return loaded;
    size_t n = 0;
    golem_status st = golem_workflow_inputs(s, dw_text(c, "selection_id"), kind, &source, budget,
                                            NULL, 0, &n, NULL);
    ...
    if (st != GOLEM_OK)
        return st;
    st =
        golem_workflow_inputs(s, dw_text(c, "selection_id"), kind, &source, budget, b, n, &n, NULL);
    struct json_object *m = NULL;
    if (st == GOLEM_OK)
        st = golem_json_parse((golem_bytes){b, n}, GOLEM_DOCUMENT_MAX_JSON, &m);
    dw_scratch_free(s, b);
    bool found = false;
    struct json_object *direct = dw_get(m, "direct");
    for (size_t i = 0; i < json_object_array_length(direct); ++i)
        if (json_object_equal(json_object_array_get_idx(direct, i), dw_get(c, "development_plan")))
            found = true;
    ...
    for (size_t j = 0; st == GOLEM_OK && j < json_object_array_length(gates); ++j) {
        ...
        for (size_t k = 0; st == GOLEM_OK && k < json_object_array_length(cases); ++k) {
            ...
            for (size_t i = 0; i < json_object_array_length(req); ++i)
                if (strcmp(json_object_get_string(json_object_array_get_idx(req, i)),
                           dw_text(json_object_array_get_idx(cases, k), "requirement_id")) == 0)
                    declared = true;
            if (!declared)
