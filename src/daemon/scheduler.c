...
static golem_status audit(void *context, const golem_lease_event *e)
{
    ...
    for (size_t i = 0; i < 16; ++i)
        (void)snprintf(hex + 2 * i, 3, "%02x", e->lease.token.authority[i]);
    (void)snprintf(name, sizeof(name), "lease-%s-%020" PRIu64, hex, e->sequence);
    uint8_t bytes[64] = "GQLS0001";
    memcpy(bytes + 8, e->lease.token.authority, 16);
    put(bytes + 24, e->lease.token.fence, 8);
    put(bytes + 32, e->observed_ns, 8);
    put(bytes + 40, e->lease.expires_ns, 8);
    put(bytes + 48, e->sequence, 8);
    put(bytes + 56, e->type, 4);
    uint32_t crc;
    (void)golem_journal_crc32((golem_bytes){bytes, 60}, &crc);
    put(bytes + 60, crc, 4);
...
static golem_status record(void *context, golem_journal_type type, golem_bytes payload)
{
    active_job *job = context;
    uint64_t sequence;
    uint8_t frame[GOLEM_JOURNAL_HEADER_SIZE + GOLEM_JOURNAL_EVENT_SIZE];
    size_t size;
    golem_status s = golem_journal_record_encode(type, job->next_record, payload, frame,
                                                 sizeof(frame), &size, NULL);
    if (s == GOLEM_OK)
        s = gd_intent_write(job->fd, job->next_record, (golem_bytes){frame, size});
    if (s == GOLEM_OK)
        s = golem_journal_append(job->journal, type, payload, &sequence, NULL);
    if (s == GOLEM_OK && sequence != job->next_record)
        s = GOLEM_ERR_REPLAY_MISMATCH;
    if (s == GOLEM_OK)
        ++job->next_record;
    return s;
}
                            ...
                            uint64_t deadline, golem_runtime_result *out)
{
    active_job *job = context;
    char name[40];
    (void)snprintf(name, sizeof(name), "execution-%020" PRIu64, stage->sequence);
    golem_journal_event event = {.type = GOLEM_JOURNAL_STARTED,
                                 .stage = stage->stage,
                                 .attempt = stage->attempt,
                                 .attempt_sequence = stage->sequence,
                                 .outcome = GOLEM_STAGE_RUNNING};
    uint8_t bytes[GOLEM_JOURNAL_EVENT_SIZE];
    size_t size;
    golem_status s = golem_journal_event_encode(&event, bytes, sizeof(bytes), &size, NULL);
    if (s == GOLEM_OK)
        s = gd_write(job->fd, name, (golem_bytes){bytes, size});
    if (s != GOLEM_OK)
        return s;
...
golem_status golem_daemon_tick(golem_daemon *d, bool *worked)
{
    if (d == NULL || worked == NULL)
        return GOLEM_ERR_INVALID_ARGUMENT;
    if (d->busy || d->poisoned)
        return GOLEM_ERR_INVALID_STATE;
    d->busy = true;
    golem_daemon_job rows[GOLEM_DAEMON_MAX_JOBS];
    size_t count;
    ...
    for (size_t i = 0; i < count; ++i)
        if (rows[i].state == GOLEM_DAEMON_READY && rows[i].ticket > d->cursor) {
            selected = i;
            break;
        }
    if (selected == count)
        for (size_t i = 0; i < count; ++i)
            if (rows[i].state == GOLEM_DAEMON_READY) {
                selected = i;
                break;
            }
    if (selected == count) {
    ...
    active_job job = {.daemon = d, .fd = -1};
    golem_runtime_options options;
    golem_lease *lease = NULL;
    golem_lease_snapshot owned;
    bool acquired = false;
    s = gd_job_path(d->root, rows[selected].ticket, job.path);
    if (s == GOLEM_OK)
        s = gd_load(job.path, &rows[selected], &options);
    if (s == GOLEM_OK) {
        job.fd = golem_evidence_path_open(job.path, true);
        if (job.fd < 0)
            s = GOLEM_ERR_IO;
    }
    char path[GD_PATH];
    if (s == GOLEM_OK)
        s = gd_path(job.path, "journal.bin", path);
    if (s == GOLEM_OK)
    ...
    /* Intents are the separately published authority for this queue. Check the
     * locked writer snapshot before recovery can dispatch any work. */
    golem_journal_checkpoint expected, observed;
    if (s == GOLEM_OK)
        s = gd_checkpoint(job.path, &expected);
    if (s == GOLEM_OK)
        s = golem_journal_checkpoint_get(job.journal, &observed);
    if (s == GOLEM_OK &&
        (expected.records != observed.records || expected.bytes != observed.bytes ||
         memcmp(expected.chain_head.bytes, observed.chain_head.bytes, GOLEM_DIGEST_SIZE)))
        s = GOLEM_ERR_DIGEST_MISMATCH;
    if (s == GOLEM_OK)
        s = golem_runtime_recover(job.journal, &options, &runtime_ops, &job, NULL, &job.runtime);
    if (s == GOLEM_OK) {
        golem_runtime_report report;
        s = golem_runtime_report_get(job.runtime, &report);
        if (s == GOLEM_OK)
            job.next_record = 2 + 2 * report.dispatched + report.reentries;
    }
    golem_lease_ops lease_ops = {audit};
    if (s == GOLEM_OK)
        s = golem_lease_create(rows[selected].run_id, &lease_ops, &job, NULL, &lease);
    uint64_t time = 0;
    if (s == GOLEM_OK)
        s = now(&time);
    if (s == GOLEM_OK) {
        s = golem_lease_acquire(lease, "daemon", time, UINT64_C(60000000000), &owned);
        acquired = s == GOLEM_OK;
    }
    if (s == GOLEM_OK)
        s = golem_runtime_lease_bind(job.runtime, lease, &owned.token);
    if (s == GOLEM_OK)
        s = golem_runtime_step(job.runtime);
    if (acquired) {
        golem_status released = now(&time);
        if (released == GOLEM_OK)
            released = golem_lease_release(lease, &owned.token, time);
        if (s == GOLEM_OK)
            s = released;
    }
    golem_runtime_free(job.runtime);
    golem_lease_free(lease);
    golem_status closed = golem_journal_close(job.journal, NULL);
    if (s == GOLEM_OK)
        s = closed;
    golem_status persisted = GOLEM_OK;
    if (s != GOLEM_OK) {
        const char *message = golem_status_string(s);
        persisted = job.fd < 0 ? GOLEM_ERR_IO
                               : gd_write(job.fd, "attention",
