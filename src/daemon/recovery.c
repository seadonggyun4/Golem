...
{
    if (sequence == 0 || sequence > GD_MAX_RECORDS)
        return GOLEM_ERR_OVERFLOW;
    char name[40];
    (void)snprintf(name, sizeof(name), "intent-%020" PRIu64, sequence);
...
static golem_status intent_list(const char *path, uint64_t *sequences, size_t *count)
{
    ...
    while ((entry = readdir(dir)) != NULL) {
        if (strncmp(entry->d_name, "intent-", 7) != 0)
            continue;
        if (strlen(entry->d_name) != 27 || n == GD_MAX_RECORDS) {
        ...
        for (size_t i = 7; i < 27; ++i) {
            ...
            if (digit > 9 || sequence > (UINT64_MAX - digit) / 10) {
                s = GOLEM_ERR_CORRUPT_JOURNAL;
                break;
            }
...
{
    char file[GD_PATH];
    golem_status s = gd_path(path, name, file);
...
static golem_status transcript(const char *path, gd_blob *out)
{
    ...
    uint64_t sequences[GD_MAX_RECORDS];
    size_t count = 0, capacity = 0;
    golem_status s = read_named(path, "recovery.meta", 40, &marker);
    if (s == GOLEM_ERR_NOT_FOUND)
        s = GOLEM_ERR_UNSUPPORTED_VERSION;
    if (s == GOLEM_OK && (marker.size != 40 || memcmp(marker.data, "GRCV0001", 8) != 0))
        s = GOLEM_ERR_UNSUPPORTED_VERSION;
    if (s == GOLEM_OK)
        s = read_named(path, "options.bin", 32, &options);
    golem_digest digest;
    if (s == GOLEM_OK)
        s = golem_digest_bytes((golem_bytes){options.data, options.size}, &digest);
    if (s == GOLEM_OK && (options.size != 32 || memcmp(marker.data + 8, &digest, 32) != 0))
        s = GOLEM_ERR_DIGEST_MISMATCH;
    if (s == GOLEM_OK)
        s = read_named(path, "id", 95, &identity);
    if (s == GOLEM_OK && (identity.size == 0 || memchr(identity.data, 0, identity.size) != NULL))
        s = GOLEM_ERR_IDENTITY_MISMATCH;
    if (s == GOLEM_OK)
        s = read_named(path, "context.bin", GOLEM_JOURNAL_MAX_PAYLOAD, &context);
    if (s == GOLEM_OK)
        s = intent_list(path, sequences, &count);
    for (size_t i = 0; s == GOLEM_OK && i < count; ++i) {
        char name[40];
        (void)snprintf(name, sizeof(name), "intent-%020" PRIu64, sequences[i]);
        gd_blob frame = {0};
        s = read_named(path, name, GOLEM_JOURNAL_MAX_PAYLOAD + GOLEM_JOURNAL_HEADER_SIZE, &frame);
        golem_journal_record record;
        size_t consumed;
        if (s == GOLEM_OK)
            s = golem_journal_record_decode((golem_bytes){frame.data, frame.size}, &record,
                                            &consumed, NULL);
        if (s == GOLEM_OK && (consumed != frame.size || record.sequence != sequences[i]))
            s = GOLEM_ERR_CORRUPT_JOURNAL;
        if (s == GOLEM_OK && i == 0 &&
            (record.type != GOLEM_JOURNAL_CREATED || record.payload.size != context.size ||
             memcmp(record.payload.data, context.data, context.size) != 0))
            s = GOLEM_ERR_REPLAY_MISMATCH;
        if (s == GOLEM_OK && frame.size > GD_LIMIT - joined.size)
            s = GOLEM_ERR_OVERFLOW;
        if (s == GOLEM_OK && joined.size + frame.size > capacity) {
            size_t grown = capacity == 0 ? 4096 : capacity;
            while (grown < joined.size + frame.size)
                grown = grown > GD_LIMIT / 2 ? GD_LIMIT : grown * 2;
            void *allocation = realloc(joined.data, grown);
            if (allocation == NULL)
                s = GOLEM_ERR_OUT_OF_MEMORY;
            else {
                joined.data = allocation;
                capacity = grown;
            }
        }
        if (s == GOLEM_OK) {
            memcpy(joined.data + joined.size, frame.data, frame.size);
            joined.size += frame.size;
        }
        free(frame.data);
    }
    ...
    if (s == GOLEM_OK)
        s = golem_replay_create(&replay_options, NULL, &replay, NULL);
    if (s == GOLEM_OK)
        s = golem_replay_feed(replay, (golem_bytes){joined.data, joined.size}, NULL);
    if (s == GOLEM_OK)
        s = golem_replay_finish(replay, &run, NULL, NULL);
...
{
    gd_blob expected = {0};
    golem_status s = transcript(path, &expected);
    if (s == GOLEM_OK &&
        (journal.size != expected.size || memcmp(journal.data, expected.data, journal.size) != 0))
        s = GOLEM_ERR_REPLAY_MISMATCH;
    free(expected.data);
    return s;
}
...
{
    gd_blob expected = {0};
    golem_status status = transcript(path, &expected);
    golem_journal_inspection inspection;
    if (status == GOLEM_OK)
        status = golem_journal_inspect((golem_bytes){expected.data, expected.size}, &inspection);
    if (status == GOLEM_OK)
        status = inspection.stream_status;
    if (status == GOLEM_OK)
        *out = (golem_journal_checkpoint){inspection.records, inspection.valid_bytes,
                                          inspection.chain_head};
    free(expected.data);
    return status;
}
...
{
    *fatal = false;
    int dir = golem_evidence_path_open(path, true);
    if (dir < 0)
        return GOLEM_ERR_IO;
    ...
    if (s == GOLEM_OK && size > expected.size)
        s = GOLEM_ERR_REPLAY_MISMATCH;
    uint8_t bytes[8192];
    size_t offset = 0;
...
golem_status gd_recover_queue(const char *root, golem_daemon_recovery_report *out)
{
    ...
    uint64_t tickets[GOLEM_DAEMON_MAX_JOBS];
    size_t count = 0;
    ...
    for (size_t i = 0; s == GOLEM_OK && i < count; ++i) {
        ...
        if (s != GOLEM_OK)
            break;
        bool changed = false, fatal = false;
        golem_status recovered = repair(path, &changed, &fatal);
        ++report.jobs;
        if (recovered == GOLEM_ERR_JOURNAL_BUSY) {
            ++report.busy;
            continue;
        }
        if (fatal || recovered == GOLEM_ERR_OUT_OF_MEMORY) {
            s = recovered;
            break;
        }
        ...
        if (changed)
            ++report.repaired;
        golem_daemon_job job = {0};
        golem_runtime_options options;
        (void)gd_load(path, &job, &options);
        if (job.state == GOLEM_DAEMON_READY)
            ++report.ready;
        else if (job.state == GOLEM_DAEMON_COMPLETE)
            ++report.complete;
        else if (job.state == GOLEM_DAEMON_BUSY)
            ++report.busy;
        else
            ++report.attention;
    }
