...
dw_entry *dw_find(golem_document_store *s, const char *id, uint32_t revision)
{
    for (size_t i = s->count; i > 0; --i) {
        ...
        if (strcmp(dw_text(e->meta, "document_id"), id) == 0 &&
            (revision == 0 || e->result.revision == revision))
            return e;
    }
    return NULL;
}
...
{
    if (strcmp(dw_text(s->spec, "work_id"), dw_text(m, "work_id")) != 0)
        return GOLEM_ERR_IDENTITY_MISMATCH;
    if (dw_uint(m, "expected_generation") != s->count + 1)
    ...
    if (strcmp(permission, "ASK_ALWAYS") == 0)
        return GOLEM_ERR_APPROVAL_REQUIRED;
    /* This operation is a local-only immutable registration; no external effects. */
    if (s->count >= dw_uint(s->spec, "max_revisions") || s->count >= GOLEM_DOCUMENT_MAX_REVISIONS)
    ...
    if (prior) {
        if (dw_uint(m, "revision") != prior->result.revision + 1 ||
            !dw_digest(m, "supersedes", &supersedes) ||
            !dw_equal(&supersedes, &prior->result.manifest_digest))
            return GOLEM_ERR_STALE_RESULT;
        if (strcmp(dw_text(prior->meta, "kind"), dw_text(m, "kind")) != 0)
            return GOLEM_ERR_INVALID_ARGUMENT;
    } else if (dw_uint(m, "revision") != 1)
        return GOLEM_ERR_STALE_RESULT;
    struct json_object *parents = dw_get(m, "parents");
    if (json_object_array_length(parents) > GOLEM_DOCUMENT_MAX_EDGES - s->edges)
        return GOLEM_ERR_BUDGET_EXHAUSTED;
    ...
    struct json_object *req = dw_get(m, "requirement_ids"),
                       *acceptance = dw_get(s->spec, "acceptance");
    for (size_t i = 0; i < json_object_array_length(req); ++i) {
        ...
        for (size_t j = 0; j < json_object_array_length(acceptance); ++j)
            if (strcmp(id, dw_text(json_object_array_get_idx(acceptance, j), "id")) == 0)
                found = true;
        if (!found)
                      ...
                      const golem_digest *payload, const golem_digest *frame)
{
    ...
    if (!s->spec) {
        ...
        if (!dw_keys(event, keys, 3) || strcmp(dw_text(event, "type"), "work") != 0 ||
            !dw_digest(event, "spec_digest", &key))
    ...
    if (!dw_keys(event, keys, 5) || strcmp(dw_text(event, "type"), "document") != 0 ||
        ...
        !dw_digest(event, "body_digest", &entry.result.body_digest) ||
        !dw_id(dw_text(event, "idempotency_key")))
        return GOLEM_ERR_CORRUPT_JOURNAL;
    strcpy(entry.key, dw_text(event, "idempotency_key"));
    for (size_t i = 0; i < s->count; ++i)
        if (strcmp(entry.key, s->entries[i].key) == 0)
            return GOLEM_ERR_CORRUPT_JOURNAL;
    uint8_t *meta = NULL, *body = NULL;
    size_t mn = 0, bn = 0;
    st = golem_evidence_read(s->cas, &entry.request_digest, GOLEM_DOCUMENT_MAX_JSON, NULL, &meta,
                             &mn, NULL);
    if (st == GOLEM_OK)
        st = dw_meta((golem_bytes){meta, mn}, &entry.meta);
    if (st == GOLEM_OK)
        st = dw_preconditions(s, entry.meta);
    if (st == GOLEM_OK)
        st = golem_evidence_read(s->cas, &entry.result.body_digest, GOLEM_DOCUMENT_MAX_BODY, NULL,
                                 &body, &bn, NULL);
    if (st == GOLEM_OK)
        st = dw_markdown(entry.meta, (golem_bytes){body, bn});
    if (st == GOLEM_OK)
        st = ex_document(s, entry.meta, (golem_bytes){body, bn});
    free(meta);
    free(body);
    if (st == GOLEM_OK) {
        entry.result.revision = (uint32_t)dw_uint(entry.meta, "revision");
        entry.result.generation = s->count + 2;
        entry.result.manifest_digest = *payload;
        entry.result.event_digest = *frame;
...
{
    if (!root || !out || golem_allocator_validate(allocator) != GOLEM_OK)
        return GOLEM_ERR_INVALID_ARGUMENT;
    void *mem = NULL;
    golem_status st = golem_allocator_alloc(allocator, sizeof(golem_document_store), &mem);
    if (st != GOLEM_OK)
        return st;
    golem_document_store *s = mem;
    memset(s, 0, sizeof(*s));
    s->root = -1;
    s->events = -1;
    s->allocator = allocator ? *allocator : golem_allocator_default();
    s->writable = writable;
    mem = NULL;
    st =
        golem_allocator_alloc(&s->allocator, GOLEM_DOCUMENT_MAX_REVISIONS * sizeof(dw_entry), &mem);
    if (st == GOLEM_OK) {
        s->entries = mem;
        memset(mem, 0, GOLEM_DOCUMENT_MAX_REVISIONS * sizeof(dw_entry));
    }
    if (st == GOLEM_OK) {
    ...
    if (st == GOLEM_OK)
        st = dw_dir(s->root, "events", create, &s->events);
    if (st == GOLEM_OK)
        st = golem_evidence_open(root, create || writable, NULL, &s->cas, NULL);
    if (st != GOLEM_OK)
                                         ...
                                         golem_diagnostic *d)
{
    ...
    if (st == GOLEM_OK) {
        ...
        if (!dw_add(event, "schema_version", json_object_new_int(1)) ||
            ...
            !dw_add_digest(event, "spec_digest", &receipt.digest))
    ...
    if (st == GOLEM_OK)
        st = dw_apply(s, event, &payload, &frame);
...
golem_status golem_document_store_close(golem_document_store *s)
{
    if (!s)
        return GOLEM_OK;
    for (size_t i = 0; i < s->count; ++i)
        json_object_put(s->entries[i].meta);
    for (size_t i = 0; i < s->reentry_count; ++i)
                                   ...
                                   const char *key, golem_document_result *out, golem_diagnostic *d)
{
    ...
    if (s->poisoned)
        return dw_report(d, GOLEM_ERR_INVALID_STATE, "close/reopen after uncertain commit");
    struct json_object *m = NULL;
    golem_status st = dw_meta(metadata, &m);
    if (st == GOLEM_OK)
        st = dw_markdown(m, body);
    golem_digest md, bd;
    if (st == GOLEM_OK)
        st = golem_digest_bytes(metadata, &md);
    if (st == GOLEM_OK)
        st = golem_digest_bytes(body, &bd);
    if (st == GOLEM_OK)
        for (size_t i = 0; i < s->count; ++i) {
            ...
            if (!dw_equal(&md, &e->request_digest) || !dw_equal(&bd, &e->result.body_digest))
                return dw_report(d, GOLEM_ERR_IDENTITY_MISMATCH,
                                 "idempotency key already binds different bytes");
            golem_document_result result = e->result;
            result.projection_status = dw_project(s, e, true);
            *out = result;
            return dw_report(d, GOLEM_OK, "committed registration, not semantic acceptance");
        }
    if (st == GOLEM_OK)
        st = dw_preconditions(s, m);
    if (st == GOLEM_OK)
        st = re_guard(s, dw_text(m, "kind"), true);
    if (st == GOLEM_OK)
        st = ex_required(s, m);
    if (st == GOLEM_OK)
        st = ex_document(s, m, body);
    struct json_object *event = NULL;
    golem_receipt receipt;
    golem_digest payload, frame;
    if (st == GOLEM_OK)
        st = golem_evidence_put(s->cas, body, &receipt, NULL);
    if (st == GOLEM_OK)
        st = golem_evidence_put(s->cas, metadata, &receipt, NULL);
    if (st == GOLEM_OK) {
        event = json_object_new_object();
        if (!dw_add(event, "schema_version", json_object_new_int(1)) ||
            !dw_add(event, "type", json_object_new_string("document")) ||
            !dw_add_digest(event, "metadata_digest", &md) ||
            !dw_add_digest(event, "body_digest", &bd) ||
            !dw_add(event, "idempotency_key", json_object_new_string(key)))
            st = GOLEM_ERR_OUT_OF_MEMORY;
    }
    if (st == GOLEM_OK)
        st = dw_put_json(s, event, &payload);
    ...
    if (st == GOLEM_OK) {
        ...
        strcpy(e.key, key);
        e.request_digest = md;
        e.result = (golem_document_result){
            s->count + 2, (uint32_t)dw_uint(e.meta, "revision"), bd, payload, frame, GOLEM_OK};
        s->edges += json_object_array_length(dw_get(e.meta, "parents"));
        s->entries[s->count++] = e;
        s->last = frame;
        ++s->event_count;
        golem_document_result result = e.result;
        result.projection_status = dw_project(s, &s->entries[s->count - 1], true);
        *out = result;
    }
    if (st == GOLEM_ERR_IO)
        s->poisoned = true;
    ...
    return dw_report(d, st,
                     st == GOLEM_OK ? "committed registration, not semantic acceptance" : NULL);
}
static golem_status lookup(golem_document_store *s, const char *id, uint32_t revision,
                           dw_entry **out)
{
                                    ...
                                    golem_document_result *out, golem_diagnostic *d)
{
    if (!out)
        return dw_report(d, GOLEM_ERR_INVALID_ARGUMENT, NULL);
    dw_entry *e = NULL;
    golem_status st = lookup(s, id, revision, &e);
    if (st == GOLEM_OK) {
        uint64_t size;
    ...
    if (st == GOLEM_OK) {
        golem_document_result result = e->result;
        result.projection_status = dw_project(s, e, false);
        *out = result;
    }
    return dw_report(d, st, NULL);
}
static golem_status content(golem_document_store *s, const char *id, uint32_t revision,
                            void *buffer, size_t capacity, size_t *required, bool metadata,
                            golem_diagnostic *d)
{
    ...
    size_t n = 0;
    if (st == GOLEM_OK)
        st = golem_evidence_read(s->cas, metadata ? &e->request_digest : &e->result.body_digest,
                                 metadata ? GOLEM_DOCUMENT_MAX_JSON : GOLEM_DOCUMENT_MAX_BODY, NULL,
                                 &body, &n, NULL);
    if (st == GOLEM_OK) {
        *required = n;
        if (capacity < n)
...
golem_status golem_document_project(golem_document_store *s, const char *id, uint32_t revision,
                                    golem_diagnostic *d)
{
    dw_entry *e = NULL;
    golem_status st = lookup(s, id, revision, &e);
    if (st == GOLEM_OK && !s->writable)
        st = GOLEM_ERR_POLICY_DENIED;
    if (st == GOLEM_OK)
        st = dw_project(s, e, true);
    return dw_report(d, st, NULL);
}
