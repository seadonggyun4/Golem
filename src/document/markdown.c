...
{
    if(strcmp(kind,"planning")==0) return "Acceptance";
    if(strcmp(kind,"ux")==0) return "Flows";
    if(strcmp(kind,"publishing")==0) return "Interface";
    if(strcmp(kind,"development-plan")==0) return "Changes";
    if(strcmp(kind,"development-result")==0) return "Results";
    if(strcmp(kind,"qa-plan")==0) return "Cases";
    if(strcmp(kind,"qa-result")==0) return "Results";
    if(strcmp(kind,"completion")==0) return "Outcome";
    if(strcmp(kind,"research")==0) return "Sources";
    if(strcmp(kind,"discovery")==0) return "Observations";
    if(strcmp(kind,"scope")==0) return "Selection";
    if(strcmp(kind,"stage-selection")==0) return "Stages";
    return "Failure";
}
static bool utf8(golem_bytes b)
{
    for(size_t i=0;i<b.size;) {
        uint32_t cp=b.data[i++], min=0; unsigned n=0;
        if(cp<0x80) { if(cp==0 || (cp<32 && cp!='\t' && cp!='\r' && cp!='\n')) return false; continue; }
        if(cp>=0xc2 && cp<=0xdf) { n=1; cp&=31; min=0x80; }
        else if(cp>=0xe0 && cp<=0xef) { n=2; cp&=15; min=0x800; }
        else if(cp>=0xf0 && cp<=0xf4) { n=3; cp&=7; min=0x10000; }
        else return false;
        if(n>b.size-i) return false;
        while(n--) { unsigned c=b.data[i++]; if((c&0xc0)!=0x80) return false; cp=(cp<<6)|(c&63); }
...
{ (void)t; (void)detail; (void)ctx; return 0; }
...
static bool placeholder(const char *s,size_t n)
{
    ...
    for(size_t i=0;i<sizeof(words)/sizeof(words[0]);++i) {
        ...
        bool same=true;
        for(size_t j=0;j<n;++j) if(tolower((unsigned char)s[j])!=words[i][j]) same=false;
        if(same) return true;
    }
    return false;
}
static int text(MD_TEXTTYPE t,const MD_CHAR *s,MD_SIZE n,void *ctx)
{
    ...
    if(c->section<0 || (t!=MD_TEXT_NORMAL && t!=MD_TEXT_CODE)) return 0;
    struct json_object *req=dw_get(c->meta,"requirement_ids");
    for(size_t i=0;i<n;) {
        if(!token_char((unsigned char)s[i])) { ++i; continue; }
        size_t start=i;
        while(i<n && token_char((unsigned char)s[i])) ++i;
        size_t len=i-start;
        if(t==MD_TEXT_NORMAL && !placeholder(s+start,len)) c->useful[c->section]+=len;
        if(c->section==5) for(size_t k=0;k<json_object_array_length(req);++k) {
            const char *id=json_object_get_string(json_object_array_get_idx(req,k));
            if(strlen(id)==len && memcmp(id,s+start,len)==0) c->requirements[k]=true;
        }
    }
    return 0;
}
...
{
    if(!b.data || !b.size || b.size>GOLEM_DOCUMENT_MAX_BODY || !utf8(b)) return GOLEM_ERR_PARSE;
    md_contract c={0}; c.meta=meta; c.section=-1;
    MD_PARSER p={0}; p.enter_block=enter_block; p.leave_block=leave_block;
    p.enter_span=enter_span; p.leave_span=leave_span; p.text=text;
    if(md_parse((const char *)b.data,(MD_SIZE)b.size,&p,&c)!=0 || c.bad || c.titles!=1) return GOLEM_ERR_REQUIREMENTS_UNMET;
    for(size_t i=0;i<10;++i) if(!c.seen[i] || c.useful[i]<8) return GOLEM_ERR_REQUIREMENTS_UNMET;
    struct json_object *parents=dw_get(meta,"parents"), *req=dw_get(meta,"requirement_ids");
    for(size_t i=0;i<json_object_array_length(parents);++i) if(!c.parents[i]) return GOLEM_ERR_REQUIREMENTS_UNMET;
    for(size_t i=0;i<json_object_array_length(req);++i) if(!c.requirements[i]) return GOLEM_ERR_REQUIREMENTS_UNMET;
    return GOLEM_OK;
}
