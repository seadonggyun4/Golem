...
int golem_cli_candidate(int argc, char **argv)
{
    ...
    bool diff = argc == 6 && !strcmp(argv[2], "diff");
    if (!validate && !gates && !status && !diff) {
        fputs("usage: golem candidate validate MANIFEST.json\n"
              ...
              "       golem candidate diff PARENT_WORK GROUP_ID CANDIDATE_ID\n"
    ...
    if (validate) {
        ...
        if (st == GOLEM_OK && (status || diff)) {
            for (int argument = 4; argument <= (diff ? 5 : 4); ++argument) {
                size_t n = strlen(argv[argument]);
                if (!n || n > 24)
                    st = GOLEM_ERR_PARSE;
                for (size_t i = 0; st == GOLEM_OK && i < n; ++i)
                    if (!((argv[argument][i] >= 'a' && argv[argument][i] <= 'z') ||
                          (argv[argument][i] >= 'A' && argv[argument][i] <= 'Z') ||
                          (argv[argument][i] >= '0' && argv[argument][i] <= '9') ||
                          argv[argument][i] == '-' || argv[argument][i] == '_'))
                        st = GOLEM_ERR_PARSE;
            }
            char request[192];
            int size =
                diff ? snprintf(request, sizeof(request),
                                "{\"operation\":\"diff\",\"group_id\":\"%s\",\"candidate\":\"%s\"}",
                                st == GOLEM_OK ? argv[4] : "", st == GOLEM_OK ? argv[5] : "")
                     : snprintf(request, sizeof(request),
                                "{\"operation\":\"status\",\"group_id\":\"%s\"}",
                                st == GOLEM_OK ? argv[4] : "");
            if (st == GOLEM_OK)
